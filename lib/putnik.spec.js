...
test('putnik: printFromDb returns empty for unknown file', (t) => {
    const instance = createPutnik();
    const result = instance.print('unknown.js');
    const expected = '';
...
test('putnik: writer handles BooleanLiteral', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    ...
    t.equal(readAst(db, 'bool.js').program.body[0].declarations[0].init.value, 1);
    t.end();
});

test('putnik: writer handles NullLiteral', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    ...
    t.ok(readAst(db, 'null.js'));
    t.end();
});

test('putnik: writer handles RegExpLiteral: pattern', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    ...
    const ast = readAst(db, 're.js');
    const node = ast.program.body[0].declarations[0].init;
    
    t.equal(node.pattern, 'test');
    t.end();
});

test('putnik: writer handles RegExpLiteral: flags', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    writeAst(db, parse('const a = /test/gi;', {
    ...
    t.equal(node.flags, 'gi');
    t.end();
});

test('putnik: writer handles BigIntLiteral', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    ...
    t.ok(readAst(db, 'bigint.js'));
    t.end();
});

test('putnik: writer handles TemplateLiteral', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    writeAst(db, parse('const a = `hello`;', {
    ...
    t.ok(readAst(db, 'tmpl.js'));
    t.end();
});

test('putnik: writer handles unary expression operator', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    writeAst(db, parse('const a = -1;', {
    ...
    const ast = readAst(db, 'unary.js');
    const node = ast.program.body[0].declarations[0].init;
    
    t.equal(node.operator, '-');
    t.end();
});

test('putnik: writer handles unary expression prefix', (t) => {
    const db = createDb();
    ...
    const ast = readAst(db, 'unary2.js');
    const result = ast.program.body[0].declarations[0].init.prefix;
    
    t.ok(result);
    t.end();
});

test('putnik: transform with fix: false returns original', async (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
...
test('putnik: printFromDb returns empty for nonexistent file', (t) => {
    const instance = createPutnik();
    const result = instance.print('unknown.js');
    const expected = '';
...
test('reader: returns null for empty file after deleting root', (t) => {
    const db = createDb();
    createAllTables(db);
    createView(db);
    writeAst(db, parse('const a = 1;', {
    ...
    db.run('DELETE FROM Program WHERE file = ?', ['empty2.js']);
    
    t.notOk(readAst(db, 'empty2.js'));
    t.end();
});

test('putnik: writer handles FunctionExpression', (t) => {
    const db = setup();
    writeAst(db, parse('const f = function() {};', {sourceType: 'module'}), 'fe.js');
    const row = db.get('SELECT async FROM FunctionExpression WHERE file = ?', ['fe.js']);
    
    t.equal(row.async, 0);
    t.end();
});

test('putnik: writer handles ClassPrivateMethod', (t) => {
    const db = setup();
    writeAst(db, parse('class A { #foo() {} }', {sourceType: 'module'}), 'cpm.js');
    const row = db.get('SELECT kind FROM ClassPrivateMethod WHERE file = ?', ['cpm.js']);
    
    t.equal(row.kind, 'method');
    t.end();
});

test('putnik: writer handles ObjectMethod', (t) => {
    const db = setup();
    writeAst(db, parse('const o = { foo() {} };', {sourceType: 'module'}), 'om.js');
    const row = db.get('SELECT kind FROM ObjectMethod WHERE file = ?', ['om.js']);
    
    t.equal(row.kind, 'method');
    t.end();
});

test('putnik: writer handles ClassProperty', (t) => {
    const db = setup();
    writeAst(db, parse('class A { foo = 1; }', {sourceType: 'module'}), 'cp.js');
    const row = db.get('SELECT computed FROM ClassProperty WHERE file = ?', ['cp.js']);
    
    t.equal(row.computed, 0);
    t.end();
});

test('putnik: writer handles OptionalMemberExpression', (t) => {
    const db = setup();
    writeAst(db, parse('a?.b;', {sourceType: 'module'}), 'ome.js');
    const row = db.get('SELECT computed FROM OptionalMemberExpression WHERE file = ?', ['ome.js']);
