...
test('putnik: print no ast', async (t) => {
...
test('putnik: readAst srcType default', async (t) => {
    ...
    const {sourceType} = (await readAst(db, 'index.js')).program;
    
    t.equal(sourceType, 'module');
    t.end();
});
...
test('putnik: writer import', async (t) => {
    const db = await parse('index.js', 'import fs from "fs";');
...
test('putnik: writer export', async (t) => {
    const db = await parse('index.js', 'const a = 1; export { a };');
...
test('putnik: writer jsx br', async (t) => {
    const db = await parse('index.js', 'const a = <br />;');
...
test('putnik: writer jsx id', async (t) => {
    const db = await parse('index.js', 'const a = <div></div>;');
...
test('putnik: writer jsx text', async (t) => {
    const db = await parse('index.js', 'const a = <div>hello</div>;');
...
test('putnik: writer private prop', async (t) => {
    const db = await parse('index.js', 'class Foo { #x = 1; }');
...
test('putnik: run returns places', async (t) => {
    const db = await parse('t.js', 'const a = 1;\n');
    const plugin = {
...
test('putnik: run returns empty for no match', async (t) => {
    const plugin = {
...
test('putnik: createDb with sqlite dialect and connection', async (t) => {
    const db = await createDb(':memory:');
    
    t.equal(db.dialect, 'sqlite');
    t.end();
});

test('putnik: createDb with sqlite throws on bad connection', async (t) => {
    const [error] = await tryToCatch(createDb, 'bad://not-a-path');
