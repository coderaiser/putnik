...
export const report = () => 'Replace withNamed with portable sequential inserts';
export const include = () => ['withNamed(__args)'];
...
export const fix = (path) => {
    ...
    const aliases = new Map(
        namedQueries.map((node) => [node.left.name, `${node.left.name}_id`]),
    );
    ...
    const statements = [
        ...namedQueries.map((node) => {
            ...
            const returningIdx = insertNode.arguments.findIndex(
                (a) => a.callee?.name === 'returning',
            );

            if (returningIdx >= 0) {
                const returningNode = insertNode.arguments[returningIdx];
                returningNode.arguments = [
                    callExpression(identifier('as'), [identifier('id'), stringLiteral(alias)]),
                ];
            }
