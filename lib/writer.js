...
const writeNode = async (db, node, file, parentId, parentType, parentField) => {
    ...
    await writeComments(db, node, file, id, type);
...
export const writeComments = async (db, node, file, id, type) => {
    const store = async (comment, flag) => {
        if (!comment.loc)
            return;
        
        await db.run(`
            INSERT INTO Comments (
                file, node_id, node_type, type, value,
                leading, trailing, inner, start_line, start_col, end_line, end_col
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT (file, start_line, start_col) DO UPDATE SET
                leading = leading | excluded.leading,
                trailing = trailing | excluded.trailing,
                inner = inner | excluded.inner
        `, [
            file,
            id,
            type,
    ...
    if (node.leadingComments)
        for (const comment of node.leadingComments)
            await store(comment, 'leading');
    
    if (node.trailingComments)
        for (const comment of node.trailingComments)
            await store(comment, 'trailing');
    
    if (node.innerComments)
        for (const comment of node.innerComments)
            await store(comment, 'inner');
};
