...
const FIX_STATEMENT_CALLEES = new Set(['update', 'deleteFrom']);
...
export const filter = (path) => {
    ...
    const resolvedSectionNode = isCallExpression(sectionPath.node) && isIdentifier(sectionPath.node.callee, {name: 'section'})
        ? sectionPath.node
        : isCallExpression(maybeSectionPath?.node) && isIdentifier(maybeSectionPath.node.callee, {name: 'section'})
            ? maybeSectionPath.node
            : null;
