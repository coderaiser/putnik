...
import {convertImperative} from '../../../imperative/imperative.js';
...
const getDeclarator = (path) => {
    ...
    if (!declarator?.id || !isIdentifier(declarator.id))
        return null;

    if (!SECTIONS[declarator.id.name])
        return null;
...
const buildReport = (message) => {
    const arg = assignmentExpression('=', identifier('message'), stringLiteral(message));
...
const buildImperative = (bodyPath) => {
    const blockSource = bodyPath.toString();
    const innerSource = blockSource.slice(1, -1);
    const js = convertImperative(innerSource);
    const expression = parse(js).program.body[0].expression;
