...
export const fix = (path) => {
    ...
    const hasFrom = isCallExpression(lastArg) && isIdentifier(lastArg.callee, {
        name: 'from',
    });
    ...
    const {__b} = getTemplateValues(args.at(-1), 'from(__a as __b)');
    
    if (__b) {
        const alias = identifier(__b.literal.value);
        
        args.splice(args.length - 1, 0, memberExpression(alias, startLine), memberExpression(alias, startCol));
