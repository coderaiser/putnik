...
const getPrimaryAlias = (selectArgs) => {
    const fromArg = selectArgs.find((a) =>
        isCallExpression(a) && isIdentifier(a.callee, {name: 'from'}));
    
    if (!fromArg)
        return null;
    
    const firstFromArg = fromArg.arguments[0];
...
export const include = () => ['CallExpression'];
...
export const filter = (path) => {
    ...
    const whereArg = path.node.arguments[0];
