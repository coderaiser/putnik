...
export const filter = (path) => {
    if (process.env.TRACE) console.error('TRACE top', JSON.stringify(require('node:path')));
    ...
    if (!isCallExpression(selectPath.node))
        return process.env.TRACE ? (console.error('TRACE line50'), false) : false;
    
    if (!isIdentifier(selectPath.node.callee, {name: 'select'}))
        return process.env.TRACE ? (console.error('TRACE line53'), false) : false;
