...
export const filter = (path) => {
    ...
    const parentIsFrom = isCallExpression(parentNode) && isIdentifier(parentNode.callee, {
        name: 'from',
    });
    
    const selectPath = parentIsFrom ? path.parentPath.parentPath : path.parentPath;
...
export const fix = (path) => {
    ...
    const alias = parentIsFrom ? getTemplateValues(path.parentPath, 'from(__a as __b)')?.__b?.literal?.value : null;
