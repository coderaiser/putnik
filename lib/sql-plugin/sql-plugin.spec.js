...
import {validatePlugin, loadSqlPlugin, loadPlugin, hasWhereClause} from '../putnik.js';
...
test('sql-plugin: hasWhereClause true with WHERE', (t) => {
    t.ok(hasWhereClause(`UPDATE VariableDeclaration SET kind = 'let' WHERE kind = 'const'`));
    t.end();
});

test('sql-plugin: hasWhereClause false without WHERE', (t) => {
    t.notOk(hasWhereClause(`UPDATE VariableDeclaration SET kind = 'let'`));
    t.end();
});

test('sql-plugin: hasWhereClause false for non-call statement', (t) => {
    t.notOk(hasWhereClause(''));
    t.end();
});
