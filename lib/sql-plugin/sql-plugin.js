...
const {file, program, expressionStatement, arrayExpression} = types;
...
const extractSections = (sql) => {
    ...
    for (const el of elements) {
        ...
        const inner = el.arguments[1];
        const wrapped = file(program([expressionStatement(arrayExpression([inner]))]));
