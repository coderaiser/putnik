...
const injectFileIntoFix = (plugin) => {
    if (!plugin.fix)
        return plugin;
    
    const queries = maybeArray(plugin.fix);
    
    const injected = queries.map((query) => {
        ...
        return query.replace(/\bWHERE\b/i, `WHERE file = ':file' AND`);
    });
    
    plugin.fix = Array.isArray(plugin.fix) ? injected : injected[0];
