...
import {createAllTables, createView, writeAst, readAst} from './putnik.js';
...
test('writer: ClassPrivateMethod', async (t) => {
    const db = await setup('class Foo { #method() { return 1; } }', {sourceType: 'module', plugins: ['jsx']});
...
test('writer: ObjectMethod', async (t) => {
    const db = await setup('const o = { foo() { return 1; } };', {sourceType: 'module', plugins: ['jsx']});
    const ast = await readAst(db, 'test.js');
...
test('writer: OptionalMemberExpression', async (t) => {
    const db = await setup('const a = b?.c;', {sourceType: 'module', plugins: ['jsx']});
    const ast = await readAst(db, 'test.js');
...
test('writer: FunctionExpression', async (t) => {
    const db = await setup('const f = function() {};', {sourceType: 'module', plugins: ['jsx']});
    const ast = await readAst(db, 'test.js');
