...
test('runner: runPlugin returns [] when no match', (t) => {
    ...
    t.equal(runPlugin(db, plugin, 'index.js').length, 0);
    t.end();
});

test('runner: runPlugin report mode returns places', (t) => {
    t.equal(runPlugin(setup(), constPlugin, 'index.js').length, 1);
    t.end();
});

test('runner: runPlugin report mode returns message', (t) => {
    t.equal(runPlugin(setup(), constPlugin, 'index.js')[0].message, 'Prefer let over const');
    t.end();
});

test('runner: runPlugin report mode does not mutate db', (t) => {
    const db = setup();
    runPlugin(db, constPlugin, 'index.js');
...
test('runner: runPlugin fix mode returns empty after fix', (t) => {
    const db = setup();
    const result = runPlugin(db, constPlugin, 'index.js', {
        fix: true,
    });
