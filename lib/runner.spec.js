...
const setup = async () => {
    ...
    await createView(db);
    await writeAst(db, parse('const a = 1;', {
        sourceType: 'module',
    }), 'index.js');
...
test('runner: runPlugin report mode returns places', async (t) => {
    t.equal((await runPlugin(await setup(), constPlugin, 'index.js')).length, 1);
    t.end();
});

test('runner: runPlugin report mode returns message', async (t) => {
    t.equal((await runPlugin(await setup(), constPlugin, 'index.js'))[0].message, 'Prefer let over const');
    t.end();
});
