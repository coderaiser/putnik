...
test('runner: runPlugin array fix runs all statements in order', async (t) => {
    ...
    const result = rows.every((row) => row.kind === 'var');
...
test('runner: runPlugin merges start_line into places', async (t) => {
    const db = await parse('index.js', 'const a = 1;');
    const plugin = {
        select: `SELECT id, start_line, start_col FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
        report: `SELECT 'test' AS message FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
    };
    
    const places = await runPlugin(db, plugin, 'index.js');
    
    t.equal(places[0].position.line, 1);
    t.end();
});

test('runner: runPlugin merges start_col into places', async (t) => {
    const db = await parse('index.js', 'const a = 1;');
    const plugin = {
        select: `SELECT id, start_line, start_col FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
        report: `SELECT 'test' AS message FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
    ...
    t.equal(places[0].position.column, 0);
    t.end();
});

test('runner: runPlugin: broken query', async (t) => {
    const db = await parse('index.js', 'const a = 1;');
    const plugin = {
        select: `SELECT hello FROM VariableDeclaration`,
    };
    
    const [error] = await tryToCatch(runPlugin, db, plugin, 'index.js');
    
    t.equal(error.message, `no such column: hello: 'SELECT hello FROM VariableDeclaration'`);
    t.end();
});

test('runner: runPlugin keeps report line when select has no start_line', async (t) => {
    const db = await parse('index.js', 'const a = 1;');
    const plugin = {
        select: `SELECT id FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
        report: `SELECT 'test' AS message, 42 AS line, 7 AS col FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
    };
    
    const places = await runPlugin(db, plugin, 'index.js');
    
    t.equal(places[0].position.line, 42);
    t.end();
});

test('runner: runPlugin insert without RETURNING runs statement', async (t) => {
    const calls = [];
    const db = {
        dialect: 'postgres',
        async all(query) {
    ...
    const plugin = {
        select: `SELECT id FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
        report: `SELECT 'test' AS message, 1 AS line, 0 AS col`,
        fix: `INSERT INTO Log (file, id) VALUES (:file, :id)`,
    };
    ...
    t.equal(calls[0].type, 'run');
    t.end();
});

test('runner: runPlugin returning without alias runs statement', async (t) => {
    const calls = [];
    const db = {
        dialect: 'postgres',
        async all(query) {
    ...
    const plugin = {
        select: `SELECT id FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
        report: `SELECT 'test' AS message, 1 AS line, 0 AS col`,
        fix: `INSERT INTO Log (id) VALUES (:id) RETURNING id`,
    };
    ...
    t.equal(calls[0].type, 'run', 'RETURNING without alias should be treated as plain run');
    t.end();
});
