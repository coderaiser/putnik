...
test('runner: runPlugin converts postgres nextval fix to sqlite autoIncrement', async (t) => {
    const db = await parse('index.js', 'const a = 1;');
    const plugin = {
        select: `SELECT id FROM VariableDeclaration WHERE file = :file AND kind = 'const'`,
        report: `SELECT 'test' AS message, 1 AS line, 0 AS col`,
        fix: `CREATE TABLE users (id INTEGER DEFAULT nextval('users_id_seq') PRIMARY KEY, name TEXT)`,
    };
