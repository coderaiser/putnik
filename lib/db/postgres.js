...
export const createDb = async (connection) => {
    ...
    await sql`SELECT 1`;  // verify connection
    ...
    const exec = async (query) => { await sql.unsafe(query); };
    
    const transaction = async (fn) => { await sql.begin(fn); };
    ...
    return {
        ...
        run, all, get, insert, exec, transaction, upsert,
        end: () => sql.end(),
    };
};
