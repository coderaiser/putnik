...
test('test: all with EXISTS returns rows when type exists', async ({db, all}) => {
    await db.exec('CREATE TABLE IF NOT EXISTS Program (id INTEGER PRIMARY KEY, file TEXT)');
    await db.run('INSERT INTO Program (file) VALUES (:file)', {file: 'test.js'});
    const rows = await db.all("SELECT 'Program' AS type WHERE EXISTS(SELECT 1 FROM \"Program\" WHERE file = ?)", ['test.js']);
    await all(rows, [{type: 'Program'}]);
});

test('test: all with EXISTS returns empty when no rows', async ({db, all}) => {
    const rows = await db.all("SELECT 'Program' AS type WHERE EXISTS(SELECT 1 FROM \"Program\" WHERE file = ?)", ['missing.js']);
    await all(rows, []);
});
