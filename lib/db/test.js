...
const parseExists = (query) => {
    // Pattern: SELECT 'type' AS type WHERE EXISTS(SELECT 1 FROM "Table" WHERE file = ?)
    const match = query.match(/SELECT\s+'(\w+)'\s+AS\s+\w+\s+WHERE\s+EXISTS/i);
...
export const createDb = () => {
    ...
    const all = (query, params = {}) => {
        const existsInfo = parseExists(query);
        
        if (existsInfo) {
            const rows = getTable(existsInfo.type);
            return rows.length > 0 ? [{type: existsInfo.type}] : [];
        }
