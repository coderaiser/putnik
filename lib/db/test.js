...
const parseSelectTable = (query) => {
    // Strip EXISTS subqueries before looking for FROM
    const cleaned = query.replace(/EXISTS\s*\([^)]*\)/gi, '');
    const match = cleaned.match(/FROM\s+"?(\w+)"?/i);
    
    return match ? match[1] : null;
};

const parseSelectColumns = (query) => {
...
export const createDb = () => {
    ...
    const evalExistsUnion = (query, params) => {
        // Handles: SELECT 'Type' AS type WHERE EXISTS(SELECT 1 FROM "Table" WHERE file = ?)
        // UNION ALL repeated
        const branches = query.split(/\bUNION ALL\b/i);
        const result = [];
        let paramIndex = 0;
        
        for (const branch of branches) {
            const typeMatch = branch.match(/SELECT\s+'([^']+)'\s+AS\s+type/i);
            const tableMatch = branch.match(/EXISTS\s*\(\s*SELECT\s+1\s+FROM\s+"?(\w+)"?\s+WHERE\s+(\w+)\s*=\s*\?/i);
            
            if (!typeMatch || !tableMatch) {
                const qCount = (branch.match(/\?/g) || []).length;
            ...
            const val = Array.isArray(params) ? params[paramIndex] : Object.values(params)[paramIndex] ?? null;
            
            paramIndex++;
            
            const rows = getTable(table);
            const exists = rows.some((row) => row[col] === val);
    ...
    const all = (query, params = {}) => {
        // EXISTS-based type discovery query has no top-level FROM
        const table = parseSelectTable(query);
        
        if (!table) {
            if (/EXISTS/i.test(query))
                return evalExistsUnion(query, params);
        ...
        const selectCols = parseSelectColumns(query);
        const result = [];
