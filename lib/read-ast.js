...
import {toSnake} from './to-snake.js';
...
const parseColumnNames = (str) => {
    ...
    return new Set(str.split(',').map((s) => s.trim().split(' ')[0]));
};

const NODE_FIELDS = {
    Program:                    { array: ['body', 'directives'],             boolean: [] },
    BlockStatement:             { array: ['body', 'directives'],             boolean: [] },
    SwitchCase:                 { array: ['consequent'],                     boolean: [] },
    ClassBody:                  { array: ['body'],                           boolean: [] },
    TemplateLiteral:            { array: ['quasis', 'expressions'],          boolean: [] },
    ArrayExpression:            { array: ['elements'],                       boolean: [] },
    ObjectExpression:           { array: ['properties'],                     boolean: [] },
    CallExpression:             { array: ['arguments'],                      boolean: [] },
    NewExpression:              { array: ['arguments'],                      boolean: [] },
    OptionalCallExpression:     { array: ['arguments'],                      boolean: [] },
    SequenceExpression:         { array: ['expressions'],                    boolean: [] },
    VariableDeclaration:        { array: ['declarations'],                   boolean: [] },
    FunctionDeclaration:        { array: ['params', 'decorators'],           boolean: ['async', 'generator'] },
    FunctionExpression:         { array: ['params'],                         boolean: ['async', 'generator'] },
    ArrowFunctionExpression:    { array: ['params'],                         boolean: ['async'] },
    ClassDeclaration:           { array: ['decorators'],                     boolean: [] },
    ClassExpression:            { array: ['decorators'],                     boolean: [] },
    ClassMethod:                { array: ['params', 'decorators'],           boolean: ['async', 'generator', 'static', 'computed'] },
    ClassPrivateMethod:         { array: ['params', 'decorators'],           boolean: ['async', 'generator', 'static'] },
    ObjectMethod:               { array: ['params'],                         boolean: ['async', 'generator', 'computed'] },
    ObjectProperty:             { array: [],                                 boolean: ['computed', 'method', 'shorthand'] },
    ClassProperty:              { array: [],                                 boolean: ['static', 'computed'] },
    ClassPrivateProperty:       { array: [],                                 boolean: ['static'] },
    MemberExpression:           { array: [],                                 boolean: ['computed'] },
    OptionalMemberExpression:   { array: [],                                 boolean: ['computed'] },
    UnaryExpression:            { array: [],                                 boolean: ['prefix'] },
    UpdateExpression:           { array: [],                                 boolean: ['prefix'] },
    YieldExpression:            { array: [],                                 boolean: ['delegate'] },
    ForOfStatement:             { array: [],                                 boolean: ['await'] },
    JSXOpeningElement:          { array: [],                                 boolean: ['selfClosing'] },
    TemplateElement:            { array: [],                                 boolean: ['tail'] },
    ImportDeclaration:          { array: ['specifiers'],                     boolean: [] },
    ExportNamedDeclaration:     { array: ['specifiers'],                     boolean: [] },
    TSEnumDeclaration:          { array: ['members'],                        boolean: [] },
};
...
for (const nodeType in NODE_FIELDS) {
    if (!t.VISITOR_KEYS[nodeType])
        throw new Error(`putnik: unknown node type in NODE_FIELDS: ${nodeType}`);
}
...
for (const nodeType in t.VISITOR_KEYS) {
    ...
    for (const field of scalars) {
        ...
        const isBoolean = booleanSet ? booleanSet.has(field) : false;

        meta.scalars.push({field, alias, isBoolean});
...
export const readAst = (db, file) => {
    ...
    const build = (row) => {
        ...
        for (const [field, fieldChildren] of byField) {
            ...
            node[field] = arrayFieldSet && arrayFieldSet.has(field) ? built : built[0];
        }
